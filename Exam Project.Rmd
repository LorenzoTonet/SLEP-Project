...
data %>%
  mutate(status_lbl = factor(status, levels = c(1, 2), labels = c("censored", "dead"))) %>%
  ggplot() +
  geom_mosaic(aes(x = product(sex, status_lbl), fill = status_lbl)) +
...
p1 <- data %>%
  mutate(status_lbl = factor(status, levels = c(1, 2), labels = c("censored", "dead"))) %>%
  ggplot() +
  geom_mosaic(aes(x = product(ph.ecog, status_lbl), fill = ph.ecog), na.rm = TRUE) +
  scale_fill_discrete(guide = "none") +
  theme_minimal()
...
p2 <- data %>%
  mutate(status_lbl = factor(status, levels = c(1, 2), labels = c("censored", "dead"))) %>%
  ggplot() +
  geom_mosaic(aes(x = product(ph.karno, status_lbl), fill = ph.karno), na.rm = TRUE) +
  scale_fill_discrete(guide = "none") +
  theme_minimal()
...
p3 <- data %>%
  mutate(status_lbl = factor(status, levels = c(1, 2), labels = c("censored", "dead"))) %>%
  ggplot() +
  geom_mosaic(aes(x = product(pat.karno, status_lbl), fill = pat.karno), na.rm = TRUE) +
  scale_fill_discrete(guide = "none") +
  theme_minimal()
...
p1 <- data %>%
  mutate(status_lbl = factor(status, levels = c(1, 2), labels = c("censored", "dead"))) %>%
  ggplot(aes(x = status_lbl, y = time, fill = status_lbl)) +
...
p2 <- data %>%
  mutate(status_lbl = factor(status, levels = c(1, 2), labels = c("censored", "dead"))) %>%
  ggplot(aes(x = status_lbl, y = wt.loss, fill = status_lbl)) +
  geom_violin(draw_quantiles = c(0.25, 0.5, 0.75)) +
...
p <- data %>%
  mutate(status_lbl = factor(status, levels = c(1, 2), labels = c("censored", "dead"))) %>%
  ggplot(aes(x = time, y = wt.loss, color = status_lbl)) +
  geom_point()
...
```{r cox model}
cox_sex = coxph(Surv(time, status) ~ sex, data=original_data)
print(cox_sex)
```
...
```{r cox_age_sex model}
cox_age_sex = coxph(Surv(time, status) ~ age+sex, data=original_data)
print(cox_age_sex)
```
...
```{r cox_karno}
cox_karno = coxph(Surv(time, status) ~ pat.karno, data=original_data)
print(cox_karno)
```
...
```{r cox_karno residuals}
ggcoxzph(test.ph)
```

```{r cox_karno}
cox_ph.karno = coxph(Surv(time, status) ~ ph.karno, data=original_data)
print(cox_ph.karno)
```
...
```{r whole cox model?}
cox_karno = coxph(
  Surv(time, status) ~ age + sex + ph.ecog + ph.karno + wt.loss + age.cat , data=original_data)
summary(cox_karno)
test.ph = cox.zph(cox_karno)
print(test.ph)
print(cox_karno)
