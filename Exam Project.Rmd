...
```{r}
calkm2 <- calibrate(mod.cov2, u=400, m=20, B=25, cmethod= 'KM')
calkm3 <- calibrate(mod.cov3, u=400, m=20, B=25, cmethod= 'KM')


par(mfrow=c(1,2))
plot(calkm2, cex.lab=0.7, xlim=c(0,.9), ylim=c(0,.9))
plot(calkm3, cex.lab=0.7, xlim=c(0,.9), ylim=c(0,.9))
par(mfrow=c(1,1))
...
ROC.pat.karno<-timeROC(T=data.cl$time,delta=data.cl$status,marker=data.cl$pat.karno,cause=1, times=c(250,500,1000),iid=TRUE)
ROC.pat.karno
```
```{r timeROC pat.karno, warning=FALSE}
ROC.ph.karno<-timeROC(T=data.cl$time,delta=data.cl$status,marker=data.cl$ph.karno,cause=1, times=c(250,500,1000),iid=TRUE)
ROC.ph.karno
```
```{r timeROC ph.ecog, warning=FALSE}
ROC.ph.karno = timeROC(T=data.cl$time,delta=data.cl$status,marker=data.cl$ph.ecog,cause=1, times=c(250,500,1000),iid=TRUE)
ROC.ph.karno
```

```{r timeROC cox model, warning=FALSE}
prediction = predict(cox_karno)
ROC.cox = timeROC(T=data.cl$time,delta=data.cl$status,marker=prediction,cause=1, times=c(250,500,1000),iid=TRUE)
ROC.cox
```
